...
class CBSA2d(nn.Module):
    """
    Stable CBSA for 2D feature maps.
    Input/Output: (B, C, H, W)
    """
    def __init__(self, dim, heads=4, dim_head=16, rep_hw=8, dropout=0.0):
        super().__init__()
        inner_dim = heads * dim_head
        self.heads = heads
        self.dim_head = dim_head
        ...
        # pre-norm on channel dim (sequence format)
        self.norm = nn.LayerNorm(dim)

        self.proj = nn.Linear(dim, inner_dim, bias=False)
        self.to_out = nn.Linear(inner_dim, dim, bias=False)

        self.pool = nn.AdaptiveAvgPool2d((rep_hw, rep_hw))
        self.softmax = nn.Softmax(dim=-1)
        self.drop = nn.Dropout(dropout)

        # conservative learnable steps
        self.step_x = nn.Parameter(torch.zeros(heads, 1, 1))
        self.step_rep = nn.Parameter(torch.zeros(heads, 1, 1))

    def attention(self, q, k, v):
        # q: (B,H,Nq,D), k/v: (B,H,Nk,D)
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale
        # numerical stability
        dots = dots - dots.amax(dim=-1, keepdim=True)
    ...
    def forward(self, x):
        ...
        # (B,C,H,W) -> (B,N,C)
        xs = rearrange(x, 'b c h w -> b (h w) c')
        xs = self.norm(xs)
        proj = self.proj(xs)  # (B,N,inner)

        # representation tokens from pooled map
        p2d = proj.reshape(b, h, w, self.inner_dim).permute(0, 3, 1, 2)   # (B,inner,H,W)
        rep = self.pool(p2d)                                               # (B,inner,R,R)
        rep = rep.reshape(b, self.inner_dim, -1).permute(0, 2, 1)         # (B,R*R,inner)

        # to multi-head
        proj = proj.reshape(b, n, self.heads, self.dim_head).permute(0, 2, 1, 3)     # (B,H,N,D)
        rep = rep.reshape(b, self.rep_hw * self.rep_hw, self.heads, self.dim_head).permute(0, 2, 1, 3)  # (B,H,R2,D)

        # cross-attn: rep <- proj
        rep_delta, attn = self.attention(rep, proj, proj)

        step_rep = torch.tanh(self.step_rep) * 0.1
        step_x = torch.tanh(self.step_x) * 0.1
        rep = rep + step_rep * rep_delta

        # self-attn on rep, then project back to spatial tokens
        x_delta, _ = self.attention(rep, rep, rep)                        # (B,H,R2,D)
        x_delta = torch.matmul(attn.transpose(-1, -2), x_delta)           # (B,H,N,D)
        x_delta = step_x * x_delta

        # merge heads
        x_delta = rearrange(x_delta, 'b h n d -> b n (h d)')              # (B,N,inner)
        x_out = self.to_out(x_delta)                                       # (B,N,C)
        x_out = rearrange(x_out, 'b (h w) c -> b c h w', h=h, w=w)
...
class C3k_CBSA(nn.Module):
    """
    C3k with CBSA branch replacing Bottleneck stack (stable version).
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3, heads=4, dim_head=16, rep_hw=8):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        ...
        blocks = []
        for _ in range(n):
            # local conv + global attention,更稳
            blocks.append(Bottleneck(c_, c_, shortcut=shortcut, g=g, k=(k, k), e=1.0))
            blocks.append(CBSA2d(c_, heads=heads, dim_head=dim_head, rep_hw=rep_hw))
...
class C3k2_CBSA(nn.Module):
    """
    C2f-style container + (Bottleneck/CBSA or C3k_CBSA) blocks.
    Fully compatible forward with C2f/C3k2 behavior.
    """
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True, heads=4, dim_head=16, rep_hw=8):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        ...
        self.m = nn.ModuleList([
            C3k_CBSA(self.c, self.c, n=1, shortcut=shortcut, g=g, e=1.0, k=3,
                     heads=heads, dim_head=dim_head, rep_hw=rep_hw) if c3k
            else nn.Sequential(
                Bottleneck(self.c, self.c, shortcut=shortcut, g=g, k=(3, 3), e=1.0),
                CBSA2d(self.c, heads=heads, dim_head=dim_head, rep_hw=rep_hw)
            )
            for _ in range(n)
        ])
